...
/**
 * GET_THRESHOLD 慢数据通道集成测试。
 *
 * <p>使用真实 Handler + 真实 Service + Stub Repository + Stub FSU Client，
 * 验证 GET_THRESHOLD 全链路的正确性和登录态校验。</p>
 */
class GetThresholdSlowChannelIntegrationTest {
    ...
    @Test
    void shouldQueryThresholdWhenLoggedIn() {
        loginService.login("FSU-001", null);

        CommandResult result = handler.handle(createGetThresholdContext("FSU-001", List.of("TEMP-001")));
    ...
    @Test
    void shouldReturnThresholdValues() {
        loginService.login("FSU-001", null);

        CommandResult result = handler.handle(
                createGetThresholdContext("FSU-001", List.of("TEMP-001", "HUMI-001")));

        String xmlData = result.toXmlDataXml();
        assertNotNull(xmlData);
        assertTrue(xmlData.contains("TEMP-001"));
        assertTrue(xmlData.contains("HUMI-001"));
        assertTrue(xmlData.contains("AlarmUpper"));
        assertTrue(xmlData.contains("AlarmLower"));
    }
    ...
    void shouldFailOnMissingFsuCode() {
        ...
        CommandResult result = handler.handle(createGetThresholdContext(null, List.of("TEMP-001")));
    ...
    void shouldFailOnMissingSignalIds() {
        ...
        CommandResult result = handler.handle(createGetThresholdContext("FSU-001", List.of()));
    ...
    void shouldNotAccessRealNetwork() {
        ...
        assertTrue(result.isSuccess());
        // StubFsuServiceClient 保证不访问网络
    }

    // ==================== Helper ====================

    private CommandContext createGetThresholdContext(String fsuCode, List<String> signalIds) {
