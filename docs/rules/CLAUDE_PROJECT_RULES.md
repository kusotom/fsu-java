...
### 2.1 协议基线 (PROTOCOL-BASELINE-001, 2026-05-21)

| 版本 | 定位 | 说明 |
|------|------|------|
| **B接口2016** | **当前主开发依据 / 真实落地协议** | 真实 FSU (51051243812345) 已验证使用 2016 码表和行为 |
| B接口2024 | 未来升级版本 / 可选兼容层 | 保留实现但标注为兼容层，不作为当前默认依据 |

**强制规则**:
1. 所有真实 FSU 接入、联调、入库、轮询、告警处理优先按 B接口2016 解释
2. 任何新增 B接口命令必须先确认 2016 码表
3. 如 2016 与 2024 Code 冲突，以 2016 为当前真实设备适配优先
4. 当前真实设备 FSUID=51051243812345 已验证使用 2016 行为
5. 2024 相关实现不得删除，但需标注为兼容层 / future upgrade
6. 后续轮询任务 (LANDING-015+) 应按 2016 推进
