<template>
  <div>
    ...
    <el-row :gutter="16" class="section">
      ...
      <el-col :span="4"><el-statistic title="紧急" :value="urgentCount"><template #suffix><el-tag size="small" type="danger">URGENT</el-tag></template></el-statistic></el-col>
      <el-col :span="4"><el-statistic title="重要" :value="importantCount"><template #suffix><el-tag size="small" type="warning">IMPORTANT</el-tag></template></el-statistic></el-col>
      ...
      <el-row :gutter="12" align="middle">
        ...
        <el-col :span="4">
          <el-select v-model="filterAlarmLevel" placeholder="告警级别" clearable size="small" style="width:100%">
            <el-option label="紧急" value="URGENT" /><el-option label="重要" value="IMPORTANT" /><el-option label="警告" value="WARN" /><el-option label="提示" value="INFO" />
          </el-select>
        </el-col>
        <el-col :span="4">
    ...
    <el-card header="当前告警" class="section">
      <DataTable :data="filteredAlarms" :loading="loading" :error="error" @refresh="fetchData">
        ...
        <el-table-column prop="alarmLevel" label="级别" width="80">
          <template #default="{row}"><StatusBadge :status="levelStatus(row.alarmLevel)" /></template>
        </el-table-column>
        <el-table-column prop="alarmStatus" label="状态" width="80">
          <template #default="{row}"><StatusBadge :status="row.alarmStatus === 'ACTIVE' ? 'active' : 'inactive'" :label="row.alarmStatus" /></template>
        </el-table-column>
        <el-table-column label="告警名称/描述" min-width="180" show-overflow-tooltip>
          <template #default="{row}">{{ row.eventName || row.alarmMeaning || row.alarmDesc || '-' }}</template>
        </el-table-column>
        <el-table-column label="发生时间" width="160">
          <template #default="{row}">{{ row.alarmTime || row.occurTime || '-' }}</template>
        </el-table-column>
...
const urgentCount = computed(() => filteredAlarms.value.filter(a => a.alarmLevel === 'URGENT').length)
const importantCount = computed(() => filteredAlarms.value.filter(a => a.alarmLevel === 'IMPORTANT').length)
...
const filteredAlarms = computed(() => {
  ...
  if (filterAlarmLevel.value) list = list.filter(a => a.alarmLevel === filterAlarmLevel.value)
  if (filterAlarmStatus.value) list = list.filter(a => a.alarmStatus === filterAlarmStatus.value)
...
function levelStatus(level: string) {
  const m: Record<string,string> = { URGENT: 'failed', IMPORTANT: 'warning', WARN: 'warning', INFO: 'info' }
  return m[level] || 'info'
}
...
async function fetchData() {
  ...
  try {
    ...
    alarms.value = Array.isArray(res?.data) ? res.data : (res?.data?.data || [])
  }
