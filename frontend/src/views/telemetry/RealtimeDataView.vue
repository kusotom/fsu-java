<template>
  <div>
    <PageHeader title="实时数据" description="查看各点位的最新采集数据" />
    <el-card>
      <el-table :data="tableData" stripe v-loading="loading">
        ...
        <el-table-column prop="valueStatus" label="状态" width="90"><template #default="{row}"><el-tag :type="row.valueStatus==='NORMAL'?'success':'warning'">{{row.valueStatus}}</el-tag></template></el-table-column>
...
const tableData = ref<RealtimeData[]>([])

onMounted(async () => { loading.value = true; try { const res: any = await getRealtimeData(); tableData.value = res.data || [] } catch { tableData.value = [] } finally { loading.value = false } })
</script>
